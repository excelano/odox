...
//
// **This is slipcase-desktop's module, taken unchanged but for the thread's
// name.** The reasoning, the measurements and the tests are that repository's,
// the same arrangement duckling's copy records: three applications carry one
// answer to this, and a published crate to share it would be a crate whose only
// contents are forty lines of D-Bus.
//
...
pub enum Scheme {
    /// The desktop asked for a dark window.
    Dark,
    /// The desktop asked for a light one, or declared no preference, which is
    /// the same answer for the reason the function below gives.
    Light,
}
...
/// Nothing to do: `winit` reports the system theme on this platform and egui is
/// already following it, so pinning a preference here would replace a live
/// answer with a fixed one.

...
# every message comes back accented and 40% longer, inside brackets. Run an
# application in it and three defects show themselves at a glance, none of which
# any test here can reach.
#
...
# release carries nothing of it. `crates/odox-ui/src/i18n.rs` is where that is
# spelled.
set -eu
...
# `msgen` fills every msgstr with its own msgid, which is the English catalogue;
# `msgfilter` then pipes each one through the transform below. Both are
# gettext's own, so this needs nothing `update-po.sh` does not.
#
# **The filter leaves `{placeholders}` alone.** They are looked up by name at run
# time, and an accented `{réäsön}` would be a placeholder no call site fills:
# the message would come out with braces in it and the defect would look like
# the pseudolocale's rather than the window's.
#
...
# the accents and the padding vanish and what comes back is the English with
# letters missing. It reads as a broken filter and is a declared encoding.
msgen "$pot" |
    ...
    msgfilter --keep-header --output-file="$out" awk '
{
    ...
    # message, and `msgfmt` refuses a msgstr ending in one where the msgid does
    # not.
    printf "%s", "[" out " " pad "]"
}
